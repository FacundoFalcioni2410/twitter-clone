...
export async function generateMetadata({ params }: { params: Promise<{ username: string }> }) {
  ...
  return { title: `People following ${user.name} (@${user.username}) · X` };
}
  ...
  return (
    <>
      ...
      <div className="flex border-b border-zinc-800">
        <a href={`/${username}/following`} className="flex-1 flex justify-center py-4 text-zinc-500 hover:bg-white/5 transition-colors">
          <span className="text-sm font-medium">Following</span>
        </a>
        <div className="flex-1 flex justify-center py-4">
          <span className="text-sm font-bold border-b-2 border-sky-500 pb-4">Followers</span>
        </div>
      </div>

      {users.length === 0 ? (
        <p className="text-zinc-500 text-center py-16">No followers yet.</p>
      ) : (
        <div>
          {users.map((user) => (
